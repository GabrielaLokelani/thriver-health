        ...
        <div className="grid lg:grid-cols-4 gap-8">
          ...
          >
            <div className="modern-card p-6 mb-6">
              <div className="flex items-center mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-white">{getCurrentUser()?.name}</h3>
                  <p className="text-sm text-warm-300">{getCurrentUser()?.bio}</p>
                </div>
              </div>
              
              <div className="grid grid-cols-2 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-electric-400">{getCurrentUser()?.followers}</div>
                  <div className="text-sm text-warm-300">Followers</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-lime-400">{getCurrentUser()?.posts}</div>
                  <div className="text-sm text-warm-300">Posts</div>
                </div>
              </div>
            </div>
            ...
            <div className="modern-card p-6">
              <h3 className="text-lg font-semibold text-white mb-4 flex items-center">
                <Users size={20} className="mr-2 text-electric-400" />
                Featured Partners
              </h3>
              <div className="space-y-4">
                {partners.filter(p => p.featured).slice(0, 3).map(partner => (
                  <div key={partner.id} className="flex items-center p-3 bg-warm-600/30 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-warm-300">{partner.specialization}</div>
                    </div>
                    {partner.verified && (
          ...
          >
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex space-x-1 bg-warm-700/50 p-1 rounded-lg">
                {[
              ...
              <div className="flex items-center space-x-4">
                <div className="relative">
                  <Search size={20} className="absolute left-3 top-1/2 transform -translate-y-1/2 text-warm-400" />
                  <input
                    type="text"
                    placeholder="Search posts, tags, users..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="pl-10 pr-4 py-2 border border-sage-200 rounded-lg focus:ring-2 focus:ring-lavender-500 focus:border-transparent bg-white text-warm-800"
                  />
                </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <select
                    ...
                    className="border border-sage-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-lavender-500 focus:border-transparent bg-white text-warm-800"
                  >
              ...
              {filteredPosts.map((post) => (
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="flex items-center">
                          ...
                          {post.verified && (
                            <CheckCircle size={16} className="text-electric-400 ml-2" />
                          )}
                          {post.authorType === 'partner' && (
                            <span className="ml-2 px-2 py-1 bg-electric-500/20 text-electric-400 text-xs rounded-full">
                              Partner
                            </span>
                          )}
                        </div>
                    ...
                    <button className="text-warm-800 hover:text-warm-600">
                      <Bookmark size={20} />
                    </button>
                  </div>
                  ...
                  {post.tags.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {post.tags.map(tag => (
                        <span
                          key={tag}
                          className="px-3 py-1 bg-warm-700/50 text-warm-300 text-sm rounded-full hover:bg-warm-600 cursor-pointer"
                        >
                          #{tag}
                        </span>
                      ))}
                  ...
                  {post.authorType === 'partner' && (
                    <div className="mb-4 p-3 bg-electric-500/20 border border-lavender-200 rounded-lg">
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-electric-400">
                          Read full article on {post.authorName}'s platform
                        </span>
                        <button className="flex items-center text-electric-400 hover:text-electric-400 text-sm">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-sage-200">
                    <div className="flex items-center space-x-6">
                      <button className="flex items-center text-warm-800 hover:text-lime-400 transition-colors">
                        <Heart size={20} className="mr-2" />
                        {post.likes}
                      </button>
                      <button className="flex items-center text-warm-800 hover:text-electric-400 transition-colors">
                        <MessageCircle size={20} className="mr-2" />
                        {post.comments.length}
                      </button>
                      <button className="flex items-center text-warm-800 hover:text-white transition-colors">
                          ...
                          <div key={comment.id} className="flex items-start space-x-3">
                            ...
                            <div className="flex-1">
                              <div className="flex items-center mb-1">
                                ...
                                <span className="text-xs text-warm-400 ml-2">{formatTimeAgo(comment.createdAt)}</span>
                              </div>
                              <p className="text-warm-300 text-sm">{comment.content}</p>
                            </div>
                          </div>
                        ))}
                        {post.comments.length > 2 && (
                          <button className="text-sm text-electric-400 hover:text-electric-400">
